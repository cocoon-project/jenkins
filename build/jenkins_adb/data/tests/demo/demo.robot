...
Check Version
    ...
    # Wait loading over
    ${is_loading_over}    Wait Until Gone    timeout=${30000}    text=Please Wait
    Should Be True    ${is_loading_over}    Loading over 10 seconds.
    # Check F-Droid text
    ${is_text_exist}    Wait For Exists    timeout=${3000}    text=F-Droid
    Should Be True    ${is_text_exist}    Text does not exist.
    Click    description=More options
    Click    text=About
    ${is_version_correct}    Wait For Exists    timeout=${2000}    text=0.66
